//! Concrete (`TensorArgs`) launch for the quant GEMV routine — used to validate
//! the kernel against the qa oracle outside the fusion path.
use cubecl::{
    prelude::*,
...
use crate::{
    ...
    routines::qa_gemv::QaGemvRoutine,
...
pub fn launch_ref<R: Runtime>(
    ...
    // Swap dims to coalesce the rhs column reads (same as the naive launch).
    ...
    let device_settings = QaGemvRoutine::device_settings(client, view_vector_sizes);
    let expand_info = QaGemvRoutine::expand_blueprint(
        &problem,
        &device_settings,
        &BlueprintStrategy::Inferred(().into()),
    )?;
    let launch_info = QaGemvRoutine::prepare(&problem, &device_settings, expand_info)?;

    let input = <InputArg<TensorArgs> as ConcreteInputsFactory<QaGemvRoutine>>::create(
        lhs,
        rhs,
    ...
    let output = <OutputArg<TensorArgs> as ConcreteOutputFactory<QaGemvRoutine>>::create(
        out,
        &launch_info.blueprint,
        &problem,
        &launch_info.vector_sizes,
    ...
    QaGemvRoutine::launch::<TensorArgs, R>(
        client,
